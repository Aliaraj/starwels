...
AC_DEFUN([STARWELS_QT_CONFIGURE],[
  ...
  dnl enable qt support
  AC_MSG_CHECKING(whether to build Starwels GUI)
  STARWELS_QT_CHECK([
    ...
    STARWELS_QT_CHECK([
      ...
      if test x$have_qt == xno && test x$starwels_qt_want_version == xauto; then
        if test x$auto_priority_version = x$qt5; then
          PKG_CHECK_MODULES([QT], [$qt4_modules], [QT_INCLUDES="$QT_CFLAGS"; have_qt=yes; QT_LIB_PREFIX=Qt; starwels_qt_got_major_vers=4], [have_qt=no])
        else
          PKG_CHECK_MODULES([QT], [$qt5_modules], [QT_INCLUDES="$QT_CFLAGS"; have_qt=yes; QT_LIB_PREFIX=Qt5; starwels_qt_got_major_vers=5], [have_qt=no])
        fi
      fi
...
AC_DEFUN([_STARWELS_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  STARWELS_QT_CHECK(AC_CHECK_LIB([z] ,[main],,STARWELS_QT_FAIL(zlib not found)))
  STARWELS_QT_CHECK(AC_CHECK_LIB([png] ,[main],,STARWELS_QT_FAIL(png not found)))

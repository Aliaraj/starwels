...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setFont(starwelsAddressFont());
#if QT_VERSION >= 0x040700
    widget->setPlaceholderText(QObject::tr("Enter a Starwels address (e.g. 1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L)"));
#endif
...
bool parseStarwelsURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!StarwelsUnits::parse(StarwelsUnits::AI, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatStarwelsURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(StarwelsUnits::format(StarwelsUnits::AI, info.amount));
        paramCount++;
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        ...
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, starwelsAppUrl, NULL, NULL);
    }

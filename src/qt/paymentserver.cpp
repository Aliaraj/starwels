...
const char* STARWELS_REQUEST_MIMETYPE = "application/starwels-paymentrequest";
const char* STARWELS_PAYMENTACK_MIMETYPE = "application/starwels-paymentack";
const char* STARWELS_PAYMENTACK_CONTENTTYPE = "application/starwels-payment";
...
//
// OSX-specific way of handling starwels: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on starwels: URIs creates FileOpen events on the Mac
    ...
    if (s.startsWith(STARWELS_IPC_PREFIX, Qt::CaseInsensitive)) // starwels: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseStarwelsURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Starwels address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom starwels addresses are not supported
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", STARWELS_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, STARWELS_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", STARWELS_PAYMENTACK_MIMETYPE);

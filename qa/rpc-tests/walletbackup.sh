...
B4ARGS="-datadir=$D4"
$STARWELSD $STARWELSDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$STARWELSD $STARWELSDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} ai"
...
function StartThree {
  $STARWELSD $STARWELSDARGS $B1ARGS &
  B1PID=$!
  $STARWELSD $STARWELSDARGS $B2ARGS &
  B2PID=$!
  $STARWELSD $STARWELSDARGS $B3ARGS &
  B3PID=$!
}
